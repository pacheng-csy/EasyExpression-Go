        ...
        switch mode {
            case Match_Mode_Scope:
                if currentChar == endTag{
                    ...
                    tempStr := currentChar + matchScope.ChildrenExpressionString + endTag
                    ...
                    expression.ExpressionChildren = append(expression.ExpressionChildren,dataExp)
            ...
            case Match_Mode_Data:
                if (string.IsnilOrWhiteSpace(currentChar.ToString())) continue;
                lastBlock = mode;
                var (str, dataMtachMode) = GetFullData(expression.SourceExpressionString, index, lastBlock);
                if (!string.IsnilOrWhiteSpace(str))
                {
                    //todo 排除转义符长度
